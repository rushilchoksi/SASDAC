...
if ($validUser == true) 
{
    ...
    if ($userAccount[6] == 1) 
    {
        $ipAddress = json_decode(file_get_contents("https://api.bigdatacloud.net/data/client-ip"), true)['ipString'];
        ...
        if ($ipReputationScore <= 1)
        {
            ...
            $geoLocationAPIURL = json_decode(file_get_contents("http://ip-api.com/json/$ipAddress"), true);
            ...
            $verificationMail->AddAddress($formData['email'], $userName);
            ...
            $filePath = $SHARED_DIRECTORY_PATH . "/sessionData.json";
            file_put_contents($filePath, json_encode(['sessionID' => $_COOKIE['PHPSESSID']]));
